...
│  DetailedOp│AICMicro│Cluster│  ComparisonAdvisorAgent           │
│  AIKG│DetailedOpV2          │  RootCauseSkillEngine             │
├─────────────────────────────────────────────────────────────────┤
...
### 7. Host-Device 堆栈关联分析

通过解析 Host 侧堆栈信息，建立算子来源追溯和根因推理。

```python
class HostDeviceCorrelator:
    """Host-Device 关联分析器"""

    def build_call_chains(self, trace_events) -> List[HostDeviceChain]:
        """
        构建 Host-Device 调用链

        关联逻辑（参考 msprof host_connect_device.py）：
        1. 按 connection_id 关联 Host API 和 Device Task
        2. 按 timestamp 对齐事件
        3. 提取堆栈信息
        """
...
class OperatorSourceClassifier:
    ...
    STACK_PATTERNS = {
        "torch_compile": ["CompiledFunctionBackward", "torch._dynamo", "torch._inductor"],
        "fusion_op": ["NPUGroupedLinearGMM", "FlashAttention", "aclnnGroupedMatmul"],
        "eager": ["aten::", "torch.ops"],
        "mindspeed": ["mindspeed.", "megatron.", "ParallelMLP", "ParallelAttention"],
        "torch_ascend": ["torch_npu", "aclnn", "AscendCL"],
        "distributed": ["torch.distributed", "HCCL", "ProcessGroup"],
    }
```
...
class RootCauseSkillEngine:
    ...
    def analyze_single(self, chains, stats, profiling_summary) -> List[RootCauseFinding]:
        """
        单版本根因推理 - 分析性能/MFU 低的原因

        内置规则：
        - excessive_small_operators: 小算子过多
        - mixed_execution_mode: eager/compile 混合
        - insufficient_fusion_operators: 融合算子不足
        - high_communication_ratio: 通信占比过高
        - poor_memory_hierarchy_utilization: 内存层次利用不佳
        """

    def analyze(self, chains_a, chains_b, diff_result) -> List[RootCauseFinding]:
        """
        对比分析根因推理 - 分析两版本差异原因

        内置规则：
        - torch_compile_fusion_issue: torch.compile 融合问题
        - eager_compile_switch: eager/compile 切换
        - communication_bottleneck: 通信瓶颈
        """
```

**Markdown Skills 格式规则库**：

```
src/skills/root_cause_analysis/
├── SKILL.md                          # Skill 主定义
├── rules/
│   ├── compile_fusion_issue.md       # torch.compile 融合问题
│   ├── eager_compile_switch.md       # eager/compile 模式切换
│   ├── communication_bottleneck.md   # 通信瓶颈
│   ├── excessive_small_operators.md  # 小算子过多
│   └── ...                           # 更多规则
└── patterns/
    ├── torch_compile_patterns.md     # torch.compile 堆栈模式
    ├── mindspeed_patterns.md         # mindspeed 堆栈模式
    └── torch_ascend_patterns.md      # torch-ascend 堆栈模式
```
...
class CommunicationMatrixAnalyzer:
    ...
    def analyze_from_db(self, db_path) -> CommunicationMatrix:
        """
        分析通信矩阵

        输出：
        - link_metrics: 每条链路的带宽利用率
        - slow_links: 慢链路列表
        - bottleneck_links: 瓶颈链路列表
        - transport_types: HCCS/RDMA 分类
        """
```
...
class LinkPerformanceDashboard:
    ...
    def generate(self, comm_matrix) -> str:
        """
        生成交互式 HTML 仪表板
...
基于 PMU 数据的 AI Core 微架构级瓶颈诊断。

```python
class InstructionAnalyzer:
    """指令级分析器"""
    # Cube/Vector/Scalar 指令混合比例、利用率、发射率

class MemoryHierarchyAnalyzer:
    """内存层次分析器"""
    # L2/UB/L0 缓存访问模式、命中率、局部性评分
